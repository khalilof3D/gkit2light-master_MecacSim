...
int Viewer::render( )
{
    ...
    gl.texture(0);
    gl.model(Translation(1, -0.2, 4) * RotationX(180)* RotationZ(90) * RotationY(-90) * Scale(0.005, 0.005, 0.005));
     //* RotationZ(90)
    ...
    gl.model(Translation(-0.9, 2, 0.9)*Scale(5,5,5));
    gl.texture(earth_texture);
    gl.draw(m_sphere);

   gl.model(Translation(0, -0.5, 0) * Scale(0.5,0.5,0.5));
    gl.texture(m_cubeMap_texture);
    gl.draw(m_cubeMap);

    //gl.texture(0);
    gl.model(Translation(0, 3, -5) * Scale(5, 5, 5));
    ...
    for(e=_Simu->_enfants.begin(); e!=_Simu->_enfants.end(); e++)
    {
        ...
        //gl.draw(m_plan);
        //gl.texture(0);
        //gl.model(Translation(0, -0.9, 1.05) * Scale(3, 3, 3));
        //gl.texture(mytunisia);
        //gl.draw(m_sphere);


        gl.texture(0);
        gl.model(RotationX(180) * Translation(-12, -1, -12) * Scale(0.12, 0.12, 0.12));
        //gl.texture(m_terrain_texture);
        gl.draw(m_terrain);
